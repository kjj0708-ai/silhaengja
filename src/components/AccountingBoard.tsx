...
export default function AccountingBoard({ adminRole }: { adminRole: 'manager' | 'treasurer' | null }) {
  ...
  if (loading) return <div className="p-10 text-center text-slate-200">불러오는 중...</div>;
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#1e293b] p-5 rounded-xl border border-slate-800 shadow-xl relative overflow-hidden group">
          ...
          <span className="text-[15px] text-slate-300 font-bold uppercase tracking-widest">현재 잔고</span>
        ...
        <div className="bg-[#1e293b] p-5 rounded-xl border border-slate-800 shadow-xl border-l-4 border-l-emerald-500">
          <span className="text-[15px] text-slate-300 font-bold uppercase tracking-widest flex items-center gap-1.5"><TrendingUp size={12} className="text-emerald-500" /> 누적 수익</span>
        ...
        <div className="bg-[#1e293b] p-5 rounded-xl border border-slate-800 shadow-xl border-l-4 border-l-rose-500">
          <span className="text-[15px] text-slate-300 font-bold uppercase tracking-widest flex items-center gap-1.5"><TrendingDown size={12} className="text-rose-500" /> 누적 지출</span>
      ...
      {adminRole === 'treasurer' || adminRole === 'manager' ? (
        <div className="bg-[#1e293b] p-6 rounded-xl border border-slate-800 shadow-xl">
          <h3 className="text-[16px] font-bold mb-5 flex items-center gap-2 text-slate-200 uppercase tracking-widest">
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
             <div className="sm:col-span-1">
              <label className="block text-[15px] font-bold text-slate-300 mb-1.5 uppercase tracking-wider">일자</label>
            ...
            <div className="sm:col-span-1">
              <label className="block text-[15px] font-bold text-slate-300 mb-1.5 uppercase tracking-wider">유형</label>
            ...
            <div className="sm:col-span-1">
              <label className="block text-[15px] font-bold text-slate-300 mb-1.5 uppercase tracking-wider">금액</label>
            ...
            <div className="flex-1">
               <label className="block text-[15px] font-bold text-slate-300 mb-1.5 uppercase tracking-wider">항목명</label>
            ...
            <div className="sm:col-span-4 flex justify-end">
               <button type="submit" className="bg-indigo-600 text-white text-[16px] font-bold px-8 py-2.5 rounded-lg hover:bg-indigo-500 uppercase tracking-widest">기록 확정</button>
            </div>
          </form>
        </div>
      ) : null}
      ...
      <div className="bg-[#1e293b] rounded-xl border border-slate-800 shadow-xl overflow-hidden">
        <div className="p-4 bg-slate-800/30 border-b border-slate-800">
           <span className="text-[15px] font-bold uppercase tracking-widest text-slate-300">회계 입출금 원장</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[18px] font-medium tracking-tight border-collapse">
            <thead className="bg-[#0f172a]/50 text-[15px] font-bold uppercase tracking-widest text-slate-300">
              <tr>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {transactions.map(tx => (
                <tr key={tx.id} className="hover:bg-indigo-900/10 transition-colors">
                  ...
                  <td className="px-2 sm:px-6 py-4 text-center">
                    <span className={`inline-block px-1.5 sm:px-2 py-0.5 rounded text-[12px] sm:text-[19px] font-bold uppercase tracking-tighter whitespace-nowrap ${tx.type === 'income' ? 'bg-emerald-900/30 text-emerald-400 border border-emerald-500/30' : 'bg-rose-900/30 text-rose-400 border border-rose-500/30'}`}>
                  ...
                  <td className={`px-3 sm:px-6 py-4 text-right font-bold font-mono whitespace-nowrap ${tx.type === 'income' ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {tx.type === 'income' ? '+' : '-'}{tx.amount.toLocaleString()} <span className="text-[19px] opacity-50 hidden sm:inline text-slate-300 ml-1">KRW</span>
                  </td>
                  {adminRole && (
                    <td className="px-3 sm:px-6 py-4">
                      {confirmDeleteId === tx.id ? (
                        <button onClick={() => handleDelete(tx.id)} className="text-[15px] font-black text-rose-500 hover:text-rose-400 bg-rose-500/10 px-2 py-1 rounded transition-colors whitespace-nowrap">
                          정말 삭제?
                        </button>
                      ) : (
              ...
              {transactions.length === 0 && (
                <tr>
                  <td colSpan={adminRole ? 5 : 4} className="px-6 py-12 text-center text-slate-300 font-mono text-[15px] uppercase tracking-widest">원장에 기록된 내역이 없습니다.</td>
                </tr>
              )}
            </tbody>
          </table>
