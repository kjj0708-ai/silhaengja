...
exports.notifyAllOnMeeting = exports.notifyAdminOnRegistration = exports.notifyAdminOnPost = void 0;
    ...
    for (let i = 0; i < valid.length; i += 500) {
        ...
        try {
            ...
            result.responses.forEach((res, idx) => {
                ...
                if (!res.success && (code === 'messaging/registration-token-not-registered' ||
...
async function getAdminTokens() {
    ...
    try {
        const authUser = await (0, auth_1.getAuth)().getUserByEmail(BOOTSTRAP_ADMIN_EMAIL);
...
// 1) 새 게시글 → 관리자·총무 알림
// ─────────────────────────────────────────────────────────────────────
exports.notifyAdminOnPost = (0, firestore_1.onDocumentCreated)({ document: 'posts/{postId}', database: DB_ID, region: REGION }, async (event) => {
    const data = event.data?.data();
    if (!data)
        return;
    const tokens = await getAdminTokens();
    const title = `📢 새 게시글 — ${data.authorName ?? ''}`;
    const body = (data.content ?? '').slice(0, 80);
    await sendPush(tokens, title, body);
});
// ─────────────────────────────────────────────────────────────────────
// 2) 모임 신청 → 관리자·총무 알림
// ─────────────────────────────────────────────────────────────────────
exports.notifyAdminOnRegistration = (0, firestore_1.onDocumentCreated)({ document: 'meeting_registrations/{regId}', database: DB_ID, region: REGION }, async (event) => {
    const data = event.data?.data();
    if (!data)
        return;
    const tokens = await getAdminTokens();
    await sendPush(tokens, `🍽️ 모임 신청 — ${data.userName ?? ''}`, '새 모임 참여 신청이 접수되었습니다.');
});
// ─────────────────────────────────────────────────────────────────────
// 3) 새 모임 등록 → 전체 회원 알림
// ─────────────────────────────────────────────────────────────────────
exports.notifyAllOnMeeting = (0, firestore_1.onDocumentCreated)({ document: 'meetings/{meetingId}', database: DB_ID, region: REGION }, async (event) => {
    const data = event.data?.data();
    if (!data)
        return;
    const tokens = await getAllTokens();
    await sendPush(tokens, `🗓️ 새 모임 공지 — ${data.title ?? ''}`, `일시: ${data.date ?? ''}`);
});
