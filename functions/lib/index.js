...
// ── 유틸: 역할별 FCM 토큰 조회 ───────────────────────────────────────
async function getTokensByRole(...roles) {
    const tokens = [];
...
// 1) 새 게시글 → 관리자·총무 알림
// ─────────────────────────────────────────────────────────────────────
exports.notifyAdminOnPost = functions
    .region(REGION)
    ...
    const body = data.title || (data.content ?? '').slice(0, 60);
    await sendPush(tokens, title, body);
});
// ─────────────────────────────────────────────────────────────────────
// 2) 모임 신청 → 관리자·총무 알림
// ─────────────────────────────────────────────────────────────────────
exports.notifyAdminOnRegistration = functions
    .region(REGION)
...
// 3) 새 모임 등록 → 전체 회원 알림
// ─────────────────────────────────────────────────────────────────────
exports.notifyAllOnMeeting = functions
    .region(REGION)
